...
DB::raw("INSERT INTO `actions`(`user`,`method`, `module`) VALUES (".$_SESSION['idx'].",'".$method."','ANNOUNCEMENT". (isset($uri[3]) ? "(".$uri[3].")" : "")."')");
$id = $uri[3];
switch ($method) {
	case 'GET':
		$result =  ($id!==null) ? DB::select("announcement",null," id=". $id) : DB::select("announcement",null,"1=1 order by date_created desc");
		echo "$result";
		break;
	case 'POST':
	...
	case 'PUT':
		...
		foreach ($data_to_insert as $key1) {
			...
			// $fields = "first_name,middle_name,last_name,designation,contact_no,email_addr,username,password,is_admin";
		echo (DB::update("announcement", join(",",$data_to_catch),join(",", $fields),"id=".$id));
		break;
	case 'DELETE':
