...
//if column is 3
	if ($this->how_many_columns==3) {
		$this->col = $col;
    	$x = 10+$col*65;
	   	$this->SetLeftMargin($x);
	   	$this->SetX($x);
	}elseif ($this->how_many_columns==2) {
		$this->col = $col;
	    $x = 10+$col*96;
	    $this->SetLeftMargin($x);
	    $this->SetX($x);
...
“I have an idea” said the merchant “I have a very special cat which has travelled with me all the way from London, and she will gobble up your rats faster than they gobbled up 
Sure enough, to the king and queen’s joy, the next time a feast was prepared and the rats appeared, the cat pounced and killed all the rats as quick as lightening.
...
function PrintChapter($num, $title, $file,$how_many_columns)
{
    // Add chapter
...
$pdf->PrintChapter(1,'A RUNAWAY REEF','http://localhost/js/10.txt',3);
$pdf->PrintChapter(2,'THE PROS AND CONS','http://localhost/js/20.txt',2);
$pdf->Output();