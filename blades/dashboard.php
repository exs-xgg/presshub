  ...
  <div class="container">
   ...
   <?php 
  $notif = json_decode(DB::raw("select concat(concat(concat(concat(name , '_'), article.id),'_'),(article.date_updated)) as name from article left join user_article on user_article.article=article.id where (is_final='Y') and user_article.user = ".$_SESSION['idx'] . " order by date_updated desc" ));
      foreach ($notif as $key => $value): ?>
        <?php foreach ($value as $key3): ?>
  ...
  $notif = json_decode(DB::raw("select nickname from issue where id not in (select distinct(issue_id) from article) and date(date_created) > DATE_SUB(NOW(), INTERVAL 5 DAY) and is_archived='N' order by date_created desc"));
      foreach ($notif as $key => $value): ?>
        <?php foreach ($value as $key3): ?>
   ...
   <?php 
  $notif = json_decode(DB::raw("select concat(concat(concat(concat(name , '_'), article.id),'_'),(article.date_updated)) as name from article left join user_article on user_article.article=article.id where (is_done='N' and body is null) and user_article.user = ".$_SESSION['idx'] ));
      foreach ($notif as $key => $value): ?>
        <?php foreach ($value as $key3): ?>
